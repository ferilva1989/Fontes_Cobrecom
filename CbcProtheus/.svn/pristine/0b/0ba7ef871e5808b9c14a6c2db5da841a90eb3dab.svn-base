...
//Variavel Cobrecom Juliana Leme
Public _ContTrans 	:= .T.
If FunName() == "SPEDNFSE"
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				Else 
					MsSeek(xFilial("SF4")+(cAliasSD1)->D1_TES)
				EndIf				
				cChaveD1 := "E" + ( cAliasSD1 )->( D1_SERIE + D1_DOC + D1_FORNECE + D1_LOJA) + PadR((cAliasSD1)->D1_ITEM,4)				
				SFT->( dbSetOrder( 1 ) )
				//utiliza a funcao SpedNatOper ( SPEDXFUN ) que possui o tratamento para a natureza da operacao/prestacao
				...
				MsSeek(xFilial("SFT")+"E"+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+PadR((cAliasSD1)->D1_ITEM,4)+(cAliasSD1)->D1_COD)
				If SFT->(FieldPos("FT_DS43080")) <> 0 .And. SFT->FT_DS43080 > 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "MG"
					nDescRed := SFT->FT_DS43080 
					nDesTotal+= nDescRed
				EndIF
				...
				*/				
				...
				If SFT->(DbSeek(xFilial("SFT")+"E"+(cAliasSD1)->(D1_SERIE+D1_DOC+D1_FORNECE+D1_LOJA)+PadR((cAliasSD1)->D1_ITEM,4)+(cAliasSD1)->(D1_COD)))
				   aadd(aCSTIPI,{SFT->FT_CTIPI})
				   //TRATAMENTO DA AQUISIÇÃO DE LEITE DO PRODUTOR RURAL CONFORME ARTIGO 207-B, INCISO II RICMS/MG
				   //PEGA OS VALORES E PERCENTUAL DO INNCENTIVO NOS ITENS NA SFT.
				   If SFT->(FieldPos("FT_PRINCMG")) > 0 .And. SFT->(FieldPos("FT_VLINCMG")) > 0
...
Endif */
	...
	_aChave := {cNota,cSerie,cClieFor,cLoja,cTipo} // By Roberto Oliveira COBRECOM

	aParam := {	aProd, cMensCli, cMensFis, aDest, aNota,;
	aInfoItem, aDupl, aTransp, aEntrega, aRetirada,;
	aVeiculo, aReboque,	aNfVincRur,	aEspVol, aNfVinc, aDetPag,;
	aProcRef, aMed, aLote, aObsCont, aPedCom, _aChave} // COBRECOM aParam 21 e 22
	...
	If ( Len(aParam) >= 5 )
		...
		EndIf	
		If ( Len(aParam) >= 18)
			// aProcRef    := aParam[18] COBRECOM
		EndIf
		if len(aParam) >= 19
			// aMed := aParam[19] COBRECOM
		endIf
		if len(aParam) >= 20
			// aLote := aParam[20] COBRECOM
		endIf
		aPedCom   	:= aParam[21] // By Roberto Oliveira COBRECOM
	EndIf
	//By Juliana Leme - 06/05/2016 COBRECOM
	If !_ContTrans //(Ponto de Entrada)
		Break
		//cJuliana
	EndIf   
Endif
