...
method setBody(aTmpData, lConsulta) class cbcIntegTprm
    ...
    if !Empty(aTmpData)
        ...
        if !(lConsulta)
            ::oBody['CriticidadeId'] := '1' // '1' - Valor Default
            ::oBody['ContatosSolicitantes'] := {}

            oContatos['Nome'] := UsrRetName(cUserID)
...
method getCliFor(cCGC) class cbcIntegTprm
    ...
    cQryCli := "SELECT A1_PESSOA [TIPO], A1_CGC [CGC], A1_NOME [NOME] FROM SA1010 WITH(NOLOCK) " // Criar um Campo ZZIDAVAL para receber o id da avaliação e consulta-lá
    cQryCli += "WHERE A1_CGC = '" + cCGC + "' "
    cQryCli += "AND A1_MSBLQL <> '1' "
    cQryCli += "AND D_E_L_E_T_ = '' "

    cQryFor := "SELECT A2_TIPO [TIPO], A2_CGC [CGC], A2_NOME [NOME] FROM SA2010 WITH(NOLOCK) " // Criar um Campo ZZIDAVAL 
...
user function zzTstIntTrpm()
    ...
    Begin Sequence
        ...
        if (oCbcIntegTprm:requestToken():isOk())
            ...
            aRet := oCbcIntegTprm:reqAvalScor(getAllCliFor()) // Deixar só essa linha dentro do "If" para Carga Inicial
