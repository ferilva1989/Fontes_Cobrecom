...
method getRgTotal(cTipoCust) class cbcModOrder
	...
	nVlrCusto 	:= ::getCustTotal(,cTipoCust)
...
method legDef() class cbcModOrder
	local aDefLeg	:= {}
	aadd(aDefLeg, {'ENCERRADO','BR_VERMELHO'})
	aadd(aDefLeg, {'BLOQUEADO VENDAS','BR_VERDE'})
	aadd(aDefLeg, {'EM FATURAMENTO','BR_AZUL'})
	aadd(aDefLeg, {'EM SEPARAÇÂO','BR_LARANJA'})
	aadd(aDefLeg, {'REGISTRO LIBERADO','BR_MARROM'})
	aadd(aDefLeg, {'REJEITADO CREDITO','BR_VERMELHO'})
	aadd(aDefLeg, {'ANALISE CREDITO','BR_VIOLETA'})
	aadd(aDefLeg, {'EM PRODUÇÂO','BR_CINZA'})
return(aDefLeg)
