...
method defFil(cBranch) class cbcXmlCte
	...
	if cBranch == '01'
		...
		::cUf 	:= 'SP'
	elseif cBranch == '02'
		::cCNPJ 	:= '02544042000208'
		::cUf	:= 'MS'
	else
		::setStatus(.F., 'Filial não configurada para baixar CTEs')
	endif
	if ::isOk()
		loadConfig(self)
	endif
...
static function saveXml(cXmlRet, cNSU, oSelf)
	...
	if ! empty(cErro)
		...
		elseIf AttIsMemberOff(oXmlResult, '_CTEPROC')
			cNomeArq := oSelf:oCfg:cXmlSavePath
			cNomeArq += oSelf:cBranch + '_'
			cNomeArq += cNSU + '_'
			cNomeArq += 'procCTE_'
...
user function zExecCte(aSch)
	...
	if aSch[1]
		...
		Processa({|| zGoCte(.T., @aRet) }, "Aguarde...", "Processando...",.F.)
	endif
