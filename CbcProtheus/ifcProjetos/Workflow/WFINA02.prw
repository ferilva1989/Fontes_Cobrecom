...
User Function WFINA02()
	...
	If MsgBox("Esta rotina envia email aos Clientes informado os Títulos a Vencer"+CRLF+;
		...
		_cParte1 += '<th width="400" bgcolor="#EEE9E9" scope="col">I.F.C. - Cobrecom - Títulos a Vencer</th>'
		_cParte1 += "</tr>"
		_cParte1 += "</table>"
		...
		_cparte4 += "<tr>"
		_cparte4 += '<th width="500"><div align="center">Acima vencimento(s) da(s) próxima(s) duplicata(s)</th>'
		_cparte4 += "</tr>"
		_cparte4 += "<tr>"
		...
		_cparte4 += "<tr>"
		_cparte4 += '<th scope="col"><div align="center">Qualquer dúvida ligar para Fone (11)-2118-3200</div></th>'
		_cparte4 += "</tr>"
		_cparte4 += "<tr>"
		_cparte4 += '<th width="500"><div align="center"> </th>'
		_cparte4 += "</tr>"
		...
		_cparte4 += '<table width="500" height="25" border="0">'
		_cparte4 += "<tr>"
		_cparte4 += '<th scope="col"><div align="center">E-mail automático - Favor não responder</div></th>'
		_cparte4 += "</tr>"
		_cparte4 += "</table>"
...
Static Function EnvMails(_dVencAte)
	...
	if oSql:hasRecords()
		...
		while oSql:notIsEof()
			...
			u_SendMail(_cTo,_cAssunto,_cBody)
		EndDo
	endif
	oSql:close()
	FreeObj(oSql)
	MsgInfo("Foram enviados "+Alltrim(Str(nQuant))+" emails contendo "+Alltrim(Str(nTitul))+ " títulos","Aviso")
Return(.T.)