...
#DEFINE MAXITEMC  048			//12	// Máxima de caracteres por linha de produtos/serviços
#DEFINE MAXMENLIN 130			//110// Máximo de caracteres por linha de dados adicionais
...
Static Function DanfeProc(oDanfe,lEnd,cIdEnt,cVal1,cVal2,lExistNfe)
	...
	local lOk		:= (Type("aNoUsrPerg") == 'A')

	if lOk
		if (lOk := !(empty(aNoUsrPerg)))
			for nX := 1 to len(aNoUsrPerg)
				&('MV_PAR' + StrZero(nX, 2)) := aNoUsrPerg[nX]
			next nX
		endif
	else	
		lOk := Pergunte("NFSIGW",.T.) .And. ( !Empty( MV_PAR06 ) .and. MV_PAR06 == 2 .Or. ( Empty( MV_PAR06 ) ))
	endif
	if lOk
...
Static Function PrtDanfe(oDanfe,oNFE,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
	...
	IF(oNF:_INFNFE:_VERSAO:TEXT >= "3.10") .AND. GetNewPar("ZZ_IMPIDES", .F.)
	...
	For nZ := 1 To nLenDet
		...
		nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 80) // Alterado de 60 para 80 - Roberto Oliveira 23/09/15
		...
		IIF((Type("oDet[nPrivate]:_Prod:_nFci") <> "U"),(AllTrim(stripTags(oDet[nX]:_Prod:_nFci:TEXT,.T.)))," "),;
		...
		.Or. !Empty(aAux[5]) .Or. !Empty(aAux[6])
			nMaxCod := MaxCod(cAuxItem, 80) // Alterado de 60 para 80 - Roberto Oliveira 23/09/15
		...
		If (Type("oNf:_infnfe:_det[nPrivate]:_Infadprod:TEXT") <> "U" .Or. Type("oNf:_infnfe:_det:_Infadprod:TEXT") <> "U") .And. ( lImpAnfav  .Or. lImpInfAd )
			...
			While !Empty(cAux)
				aadd(aItens,{;
				"",;
				SubStr(cAux, 1, nMaxDes+40),;//juliana
				"",;
				"",;
				"",;
				"",;
		...
		*/
		...
		//INICIO CUSTOMIZADO
		//IMPRESSÂO FCI - 25/05/2015
		//JULIANA LEME
		/*If (Type("oNf:_infnfe:_det[nPrivate]:_Prod:_nFci:TEXT") <> "U" .Or. Type("oNf:_infnfe:_det:_Prod:_nFci:TEXT") <> "U") //.And. ( lImpAnfav .Or. lImpInfAd )
		cAux := stripTags(AllTrim(SubStr(oDet[nX]:_Prod:_nFci:TEXT,1)), .T.)

		While !Empty(cAux)
		aadd(aItens,{;
		"",;
		SubStr("FCI: "+cAux,1,nMaxDes),;
		"",;
		"",;
		"",;
		"",;
	...
	If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
		...
		//CUSTOMIZADO
		//JULIANA LEME - 25/05/2015
		//Padrão Totvs
		While !Empty(cAux)
			aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
			cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
		EndDo
		//Padrão Customização Cobrecom
		Do While !Empty(Alltrim(cAux))
			_cBrBr := ""
			Do While Left(cAux,2) == "\\"
				_cBrBr := "\\ "
				cAux := AllTrim(Substr(cAux,3)) // Tiro "\\" do Inicio
			EndDo
			_nPsBar := At("\\",Alltrim(cAux))-1
			If _nPsBar <= 0
				_nPsBar := Len(cAux)
			EndIf
			If _nPsBar > 85 .And. (_nPsBar-85) > 5
				_nPsBar := 85
			EndIf
			cAux2 := _cBrBr+Alltrim(Left(cAux,_nPsBar))
			aadd(aMensagem,cAux2)
			cAux := AllTrim(Substr(cAux,_nPsBar+1))
		EndDo
		//FIM CUSTOMIZADO
	EndIf
	/*
	...
	*/
	...
	ElseIf cModFrete =="3"
		oDanfe:Say(nLine+397,412,"3-PROPRIO REM",oFont08:oFont)
	ElseIf cModFrete =="4"
		oDanfe:Say(nLine+397,412,"4-PROPRIO DEST",oFont08:oFont)
	ElseIf cModFrete =="9"
...
*/
	...
	//Incio Custom Cobrecom - 10/11/2016
	nBaseTxt := 230
	oDanfe:Box(nLine+042,nBaseCol,nLine+139,450)
	oDanfe:Say(nLine+057,nBaseTxt, "Identificação do emitente",oFont12N:oFont)
	If len(oEmitente:_xNome:Text)>40//43
		oDanfe:Say(nLine+070,nBaseTxt,SubStr(NoChar(oEmitente:_xNome:Text,lConverte),1,40), oFont12N:oFont )
		oDanfe:Say(nLine+080,nBaseTxt,SubStr(NoChar(oEmitente:_xNome:Text,lConverte),41,45), oFont12N:oFont )
	Else
		oDanfe:Say(nLine+070,nBaseTxt, NoChar(oEmitente:_xNome:Text,lConverte),oFont12N:oFont)
	Endif
	If len(oEmitente:_xNome:Text)>45
	...
	If nfolha>=1
		...
		If oDanfe:nDevice == IMP_PDF
			//Impressão do Logo por enquanto só para 3 Lagoas
			If lMv_Logod
				//CUSTOMIZADO
				//JULIANA LEME
				//25/05/2015
				//oDanfe:SayBitmap(005,075,cLogoD,095,096) Padrão TITVS
				oDanfe:SayBitmap(005,075,cLogoD,095,050) //Padrão Cobrecom
			Else
				//oDanfe:SayBitmap(005,075,cLogo,095,096)Padrão TITVS
				oDanfe:SayBitmap(005,075,cLogo,095,050) //Padrão CObrecom
			EndIf
		EndIf
		//FIM CUSTOMIZACAO
	Endif
...
Static Function DanfeIT(oDanfe, nLine, nBaseCol, nBaseTxt, nFolha, nFolhas, aColProd, aMensagem, nPosMsg, aTamCol)
	...
	If nFolha == 1
		...
		nAux += aTamCol[2]
		AADD(aColProd, {nAux, nAux + aTamCol[3]}) // "DESCRIÇÃO DO PRODUTOS/SERVIÇOS"
		nAux += aTamCol[3]
		...
		AADD(aColProd, {nAux, nAux + aTamCol[2]}) // "DESCRIÇÃO DO PRODUTOS/SERVIÇOS"
		nAux += aTamCol[2]
		AADD(aColProd, {nAux, nAux + aTamCol[3]}) // "FCI"
		nAux += aTamCol[3]
		...
		//	AADD(aColProd, {nAux, nAux + aTamCol[10]}) // "PER DESC"
		//	nAux += aTamCol[10]
		AADD(aColProd, {nAux, nAux + aTamCol[11]}) // "BC.ICMS"
		nAux += aTamCol[11]
		AADD(aColProd, {nAux, nAux + aTamCol[12]}) // "VLR ICMS"
		nAux += aTamCol[12]
		AADD(aColProd, {nAux, nAux + aTamCol[13]}) // "VL.IPI"
		nAux += aTamCol[13]
		AADD(aColProd, {nAux, nAux + aTamCol[14]}) // "ICMS"
		nAux += aTamCol[14]
		AADD(aColProd, {nAux, nAux + aTamCol[15]}) // "IPI"
		...
		nAux += aTamCol[2]
		oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[3])
		oDanfe:Say(nLine+462, nAux + 2,"FCI", oFont08N:oFont)
		nAux += aTamCol[3]
		oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[4])
		oDanfe:Say(nLine+462, nAux + 2,"NCM/SH", oFont08N:oFont)
		nAux += aTamCol[4]
		...
		nAux += aTamCol[11]
		oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[12])
		oDanfe:Say(nLine+462, nAux + 2,"V.ICMS", oFont08N:oFont)
		nAux += aTamCol[12]
		oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[13])
		oDanfe:Say(nLine+462, nAux + 2,"V.IPI", oFont08N:oFont)
		nAux  += aTamCol[13]
		nAux2 := nAux
		oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[14])
		oDanfe:Say(nLine+468, nAux + 2,"ICMS", oFont08N:oFont)
		...
		oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[15])
		oDanfe:Say(nLine+468, nAux + 2,"IPI", oFont08N:oFont)
		oDanfe:Box(nLine+454, nAux2, nLine+461, nAux2 + aTamCol[14] + aTamCol[15])
		oDanfe:Say(nLine+460, nAux2 + 2,"ALIQUOTA", oFont08N:oFont)
		//FIM CUSTOMIZADO
	...
	Else
		...
		If nPosMsg > 0
			...
			AADD(aColProd, {nAux, nAux + aTamCol[12]}) // "VLR ICMS"
			nAux += aTamCol[12]
			AADD(aColProd, {nAux, nAux + aTamCol[13]}) // "VALOR IPI"
			nAux += aTamCol[13]
			AADD(aColProd, {nAux, nAux + aTamCol[14]}) // "ICMS"
			nAux += aTamCol[14]
			AADD(aColProd, {nAux, nAux + aTamCol[15]}) // "IPI"
			...
			nAux += aTamCol[11]
			oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[12])
			oDanfe:Say(nLine+462, nAux + 2,"VL.ICMS", oFont08N:oFont)
			nAux += aTamCol[12]
			oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[13])
			oDanfe:Say(nLine+462, nAux + 2,"V.IPI", oFont08N:oFont)
			nAux  += aTamCol[13]
		...
		Else
			...
			nAux += aTamCol[12]
			oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[13])
			oDanfe:Say(nLine+462, nAux + 2,"VL.IPI", oFont08N:oFont)
			nAux  += aTamCol[13]
			nAux2 := nAux
			oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[14])
			oDanfe:Say(nLine+468, nAux + 2,"ICMS", oFont08N:oFont)
...
STATIC FUNCTION MaxCod(cString,nTamanho)
	...
	Local nMax	:= 0
	Local nY   	:= 0
	Default nTamanho := 60		 //Custom Juliana
...
@return aTamCol  Array contendo os tamanhos das colunas baseados nos valores.
/*/
//-----------------------------------------------------------------------
	...
	For nX := 1 To Len(aValores[1])
		...
		For nY := 1 To Len(aValores[1][nX])
			...
			If (oPrinter:GetTextWidth(aValores[1][nX][nY], oFont) * nConsTex + 2) > nAux
				...
				nAux := oFontSize:getTextWidth(Alltrim(aValores[1][nX][nY]), oFontCabec:Name, oFontCabec:nWidth, oFontCabec:Bold, oFontCabec:Italic )+ 2 //Juliana antes 4
			EndIf
		...
		If aTamCol[nX] < nAux
			If nX == 3
				// By Roberto Oliveira - Fixando em no máximo 155 pixels para a coluna FCI
				aTamCol[nX] := Min(nAux,155)
			Else
				aTamCol[nX] := nAux
			EndIf
		EndIf
